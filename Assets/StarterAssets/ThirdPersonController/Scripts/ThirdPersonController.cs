...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        public void SensitiveChange(float value)
        {
        ...
        private void CameraRotation()
        {
            ...
            // 1. 현재 추적 중인 터치가 실제로 존재하는지 실시간 검증 (가장 중요)
            if (rotatingCamera && activeTouchId != -1)
            {
                bool stillTouching = false;
                foreach (var t in touchscreen.touches)
                {
                    if (t.touchId.ReadValue() == activeTouchId && t.isInProgress)
                    {
                        stillTouching = true;
                        break;
                    }
                ...
                // 손가락이 사라졌거나 상태가 꼬였다면 즉시 초기화하여 '락' 방지
                if (!stillTouching)
                {
                    activeTouchId = -1;
            ...
            foreach (var touch in touchscreen.touches)
            {
                ...
                // [핵심] 점프/착지 중 UI 레이캐스트가 튈 수 있으므로 안전장치 추가
                ...
                {
                    // 화면 오른쪽 절반 + UI가 아닌 곳 터치 시에만 시작
                    if (touchPosition.x >= Screen.width / 2f && !isOverUI)
                    {
                ...
                else if (phase == UnityEngine.InputSystem.TouchPhase.Moved && rotatingCamera && fingerId == activeTouchId)
                {
            ...
            // 3. 최종 회전값 적용
            CinemachineCameraTarget.transform.rotation = Quaternion.Euler(currentY, currentX, 0.0f);
        }

        //private void CameraRotation()
        //{
        //    // 1. New Input System의 터치스크린 활성화 여부 확인
        //    var touchscreen = Touchscreen.current;
        //    if (touchscreen == null) return;
        ...
        //        // 만약 추적 중인 손가락이 화면에서 사라졌다면 강제 리셋
        //        if (!touchStillExists)
        //        {
        //            activeTouchId = -1;
        //            rotatingCamera = false;
        ...
        //        text.text = $"rotatingCamera : {rotatingCamera}\nactiveTouchId : {activeTouchId}";
        ...
        //                if (rotatingCamera && fingerId == activeTouchId)
        //                {
        //                    touchIdText.text = $"fingerId : {fingerId}";
        //                    Vector2 delta = touchPosition - lastTouchPosition;
        ...
        //    if (Touchscreen.current.touches.Count == 0 || !Touchscreen.current.primaryTouch.isInProgress)
        //    {
        //        rotatingCamera = false;
        //        activeTouchId = -1;
        //    }
        ...
        //    if (!rotatingCamera && _input.look != Vector2.zero)
        //    {
